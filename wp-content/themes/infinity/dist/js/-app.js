...
$('[data-hubspot-form]').each(function(){
	var hubspot_vars = {
		...
		onFormReady : function(form) {
			if( $(form).is(':visible') ) {
				$(form).find('select').each(function(){
					...
					$(this).closest('.select-wrap').prepend('<div class="placeholder">' + $(this).find('option[selected]').text() + '</div>');
				});
		...
		onFormSubmitted : function(form) {
			$('.hs-submitting .form-wrap').slideUp('slow');
			$('.hs-submitting .thank-you').slideDown('slow');
			$('.hs-submitting').removeClass('hs-submitted').addClass();
			$(window).trigger('resize');
		}
	};
	...
	// CONFIGURATOR
		...
		var quiz_to_slide = function(key) {
			...
			var check = 1;
			$('#quiz [data-quiz-page="' + current_slide + '"]').find('input, select').each(function(){
		...
		$('#quiz').on('submit', function(e){
			e.preventDefault();			
			if( quiz_to_slide( 'results' ) ) {
				$('form[data-form-id="51b185b0-63d0-49a7-98c5-df251976130e"]').trigger('submit');
		...
		var quiz_hubspot_mapping = {
			...
			'feet-problems'	: {
				'hs_field'		: 'aiac_feet_problems',
				'values'		: {
					'Bunionette'		: 'Bunionette',
					'Hammertoe(s)'		: 'Hammertoe(s)',
					'Big Toe Arthritis'	: 'Big Toe Arthritis',
					'Diabetic'			: 'Diabetic',
					'None'				: 'None'
				}
			},
			'bunion-surgery': {
				'hs_field'		: 'aiac_bunion_surgery',
				'values'		: {
					'Yes'				: 'Yes',
					'No'				: 'No'
				}
		...
		var map_hubspot_field = function( $this ) {
			if( $this.attr('name') in quiz_hubspot_mapping ) {
				if( $this.is('[type="checkbox"]') ) {
					$('#quiz [name="' + $this.attr('name') + '"]').each(function(){
						$('.hs-form-quiz [name="' + quiz_hubspot_mapping[$(this).attr('name')].hs_field + '"][value="' + quiz_hubspot_mapping[$this.attr('name')].values[$(this).val()] + '"]').prop('checked', $(this).is(':checked') );
					})
				} else if( $this.is('[type="radio"]') ) {
					$('.hs-form-quiz [name="' + quiz_hubspot_mapping[$this.attr('name')].hs_field + '"]').prop('checked', false);
					$('.hs-form-quiz [name="' + quiz_hubspot_mapping[$this.attr('name')].hs_field + '"][value="' + quiz_hubspot_mapping[$this.attr('name')].values[$this.val()] + '"]').prop('checked', $this.is(':checked') );
				} else if( $('[name="' + quiz_hubspot_mapping[$this.attr('name')].hs_field + '"]') && $this.val() in quiz_hubspot_mapping[$this.attr('name')].values ) {
					$('.hs-form-quiz [name="' + quiz_hubspot_mapping[$this.attr('name')].hs_field + '"]').val( quiz_hubspot_mapping[$this.attr('name')].values[$this.val()] );
				} else if( $('[name="' + quiz_hubspot_mapping[$this.attr('name')].hs_field + '"]') ) {
					$('.hs-form-quiz [name="' + quiz_hubspot_mapping[$this.attr('name')].hs_field + '"]').val( $this.val() );
				}
			}
		};
