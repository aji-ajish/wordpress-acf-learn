...
function infinity_setup()
{
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__('Primary', 'infinity'),
		)
	);
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'infinity_custom_background_args',
			array(
...
function acf_flexible_content_thumbnail()
{
	...
	// REGISTER ADMIN.CSS
	wp_enqueue_style('css-theme-admin', get_stylesheet_directory_uri() . '/dist/stylesheets/admin.css', false, 1.0);

	// REGISTER ADMIN.JS
	wp_register_script('js-theme-admin', get_stylesheet_directory_uri() . '/dist/javascripts/admin.js', array('jquery'), 1.0, true);
	wp_localize_script(
		'js-theme-admin',
		'theme_var',
		array(
			'upload' => get_stylesheet_directory_uri() . '/dist/images/acf-thumbnail/',
		)
	);
...
{
	$GLOBALS['content_width'] = apply_filters('infinity_content_width', 640);
}
...
function infinity_widgets_init()
{
	register_sidebar(
		array(
			...
			'id'            => 'sidebar-1',
			'description'   => esc_html__('Add widgets here.', 'infinity'),
...
{
	wp_enqueue_style('infinity-style', get_stylesheet_uri(), array(), _S_VERSION);
	wp_style_add_data('infinity-style', 'rtl', 'replace');

	wp_enqueue_script('infinity-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true);
